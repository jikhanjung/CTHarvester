# CTHarvester 버전 관리: 전략과 실전 가이드

## 개요

CTHarvester는 중앙 집중식 버전 관리 시스템을 사용합니다. 모든 버전 정보는 `version.py` 파일에서 관리되며, `scripts/bump_version.py` 스크립트를 통해 자동화된 방식으로 업데이트됩니다.

## 🎯 핵심 원칙

- **Single Source of Truth**: `version.py` 파일이 유일한 버전 정보 소스입니다.
- **Semantic Versioning**: [SemVer 2.0.0](https://semver.org/) 규칙을 엄격히 준수합니다.
- **자동화**: `scripts/bump_version.py` 스크립트를 통해 버전 업데이트와 관련 작업(CHANGELOG, Git commit/tag)을 자동화합니다.
...
| `scripts/bump_version.py` | 버전 업데이트 및 관련 작업을 자동화하는 스크립트입니다. |
| `Cargo.toml` | Rust 모듈(`ct_thumbnail`)의 버전. `version.py`와 일치해야 하며 스크립트가 함께 갱신합니다. |
| `CHANGELOG.md` | 각 버전별 변경 사항을 기록하는 문서입니다. |

## 🔧 버전 관리 명령어

`scripts/bump_version.py` 스크립트는 다음과 같은 명령어를 제공합니다.
명령 체계는 Modan2의 `manage_version.py`와 동일하게 맞췄습니다.
...
```bash
# 0.1.4 -> 0.2.0-alpha.1
python scripts/bump_version.py preminor

# 0.2.0-alpha.1 -> 1.0.0-beta.1
python scripts/bump_version.py premajor beta
```
...
python scripts/bump_version.py stage beta
```
...
1.  **v0.2.0 개발 시작 (alpha)**
    ...
    ```bash
    python scripts/bump_version.py preminor alpha
...
`scripts/bump_version.py`를 실행하면 버전업과 태그 생성을 한 번에 처리해주는 것이 가장 이상적입니다. 하지만 만약 버전업 커밋을 한 직후, 태그를 생성하기 전에 긴급한 버그 수정이 필요하다면 다음 절차를 따라야 합니다.

1.  버그를 수정하고 새로운 커밋을 만듭니다. (예: `fix: Critical bug in build process`)
2.  `scripts/bump_version.py`의 자동 태그 생성 기능을 사용하지 않습니다.
