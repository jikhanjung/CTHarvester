...
- [120](20260729_120_quality_checklist_baseline_copied_in.md) - **품질 상태표의 기준을 저장소 안으로**: 119가 열어 둔 항목을 닫음 — `TODOs.md`의 상태표가 채점 기준으로 삼던 `../Modan2/docs/CODE_QUALITY_GUIDE.md`의 Appendix A 체크리스트(+절 목록, §7, §1 규칙군 표, Appendix B)를 devlog로 **원문 복사**. 119는 "복사하지 말고 참조하라"였지만 **여기서는 성질이 반대다** — 표가 재는 것은 현재 최선의 권고가 아니라 *v1.0(2026-07-23) 대비 어디까지 왔는가*이므로 **갱신되지 않는 것이 결함이 아니라 요건**이고, 그래서 계획(`TODOs.md`)이 아니라 기록(`devlog/`)에 둔다. 공유 가이드로 기준을 옮기지 않은 추가 근거: **절 번호가 다른 것을 가리킨다**(Modan2 §7 = Packaging & release verification, 공유본 §7 = Dead code…) — 조용히 바꿔 달았다면 링크는 멀쩡한 채 뜻만 틀렸을 것. **옮기다 인용 오류 발견**: `C901` 임계값 15를 세 문서가 "the guide's threshold"라 적어 왔으나 **원문에는 숫자가 없고**(Modan2 자신은 19) 15는 가이드보다 열 달 앞선 이 저장소 고유 값(devlog 038). 옆 저장소를 열어야 확인되는 인용은 아무도 확인하지 않는다는 사례. devlog 100·103의 Modan2 경로는 *그때 무엇을 보고 감사했는가*의 기록이라 그대로 둠 — 살아 있는 상호 의존은 이제 없음
- [119](20260729_119_shared_guides_checkout.md) - **공통 가이드를 복사하지 않고 참조하기로**: 형제 프로젝트가 공유하는 규약을 `.guides/`(비공개 가이드 체크아웃을 가리키는 심볼릭 링크, `.gitignore` 대상)로 참조한다. 가이드 묶음 스스로가 *"프로젝트 `docs/`로 복사해 채택하라"*고 적고 있었는데 **작성 다음 날 그 권고 중 하나가 상류 개정으로 철회됐다** — 가이드는 스냅샷을 일반화한 문서라 원본이 뒤집혀도 모르고, **사본은 자기가 낡았다고 알려 주지 않는다.** 새로울 것은 없다는 점도 기록 — `TODOs.md:9`와 devlog 100·103이 이미 `../Modan2/docs/CODE_QUALITY_GUIDE.md`를 상대경로로 참조하고 있었고 그것도 형제가 옆에 체크아웃돼 있어야 풀린다. 달라지는 건 **소유자**(한 형제의 사본 → 어느 프로젝트에도 속하지 않는 공유 묶음). **이 저장소에 CLAUDE.md가 없어 새로 만들었다** — 링크가 gitignore되므로 있다는 사실을 알릴 것이 없고, `CONTRIBUTING.md`는 그 체크아웃을 영영 가질 수 없는 외부 기여자용이라 맞지 않는다. 내용을 지어내지 않고 이미 있는 문서들의 역할만 가리키는 **라우팅** 형태. sparse clone 함정 셋(`--no-cone` 필요 / 패턴에 앵커 `'/guides/'` / 상대경로 심볼릭 링크)과 **`--filter=blob:none`은 "안 받는다"가 아니라 "필요할 때 받는다"**(pull의 diffstat만으로도 blob이 내려온다). **열어 둠**: `TODOs.md`의 품질 상태표는 Modan2 원본의 Appendix A 체크리스트를 추적하는데 공유 가이드(7절)에는 그 체크리스트가 없어 지금 기준을 옮기면 추적이 깨진다

- [118](20260729_118_backlog_cleared_and_what_it_hid.md) - **백로그 소진, 그리고 통과하던 테스트들이 감추고 있던 것**: main을 빨갛게 만든 macOS ETA 플레이크로 시작 — 앞선 두 번의 수정이 모두 틀렸던 이유는 샘플이 순간 속도가 아니라 **누적** 속도라 초기 이상치가 smoothing window에 남기 때문. 시계를 주입해 허용오차를 `rel=0.5`에서 float 노이즈로 좁힘. `ui/widgets/`를 mypy로 들이면서 **exclude 목록이 비었고**, 결함 둘이 딸려 나옴 — `mcube_widget.py`의 `self.parent`가 `QWidget.parent()`를 덮어쓰던 것(117의 `progress_dialog.py`와 동형), `object_viewer_2d.py`의 마우스·리사이즈 핸들러 4경로가 단독 생성 뷰어에서 전부 `AttributeError`. 형식이 깨져(끝 쉼표) 처음부터 아무것도 억제하지 못하던 `type: ignore`도 발견. 속성 테스트 14개를 넣으려다 `[tool.hypothesis]`가 **hypothesis가 읽지 않는 파일의 죽은 설정**(`derandomize`가 `true`인데 실측 `False`)임을 확인해 `conftest.py` 프로파일로 이전. bandit은 75개 중 ruff가 포팅 안 한 4개를 재보고 존치 — 유일한 실질 근거는 `B613 trojansource`. 마지막으로 `test_settings_persistence`가 `hasattr(window, "settings")`(실제는 `settings_manager`) 가드로 **통과 외의 결과를 낼 수 없었고**, 격리에 쓰던 `CTHARVESTER_SETTINGS_DIR`은 앱이 읽지 않는 이름이라 **통합 테스트가 개발자의 진짜 `preferences.json`에 쓰고 있었음**. `test_basic.py`는 포섭 관계를 확인하고 삭제(CI `--ignore`도 함께). 결함 주입 프로브 5회로 전부 검증
- [117](20260728_117_mypy_over_ui.md) - **mypy 범위를 `ui/`까지**: 재보니 `ui/` 전체가 6건뿐이라 미룰 이유가 처음부터 크지 않았음. exclude돼 있던 `progress_dialog.py`의 "parent type issue"는 실제로 `self.parent`/`self.layout`이 Qt 메서드를 덮어쓰던 결함이었음. 더 중요한 발견은 CI·`make type-check`·pre-commit이 **서로 다른 mypy**(훅은 락의 1.20.2 대신 v1.8.0 + 락에 없는 스텁)를 돌고 있던 것 — 훅을 local로 바꿔 CI와 동일 명령으로 통일. 잔재였던 `docs/_build/` 24MB 정리
- [116](20260728_116_settings_out_of_the_data_directory.md) - **설정을 데이터 디렉토리 밖으로 (규약 R02)**: 115에서 설정을 로그와 같은 디렉토리에 넣은 결정을 뒤집음 — 설정은 기기 종속 상태라 백업·동기화 대상이 아니고, 설정 가능한 디렉토리 안에 설정 파일을 두면 부트스트랩 순환이 생긴다. `platformdirs`로 OS 설정 위치 해석(`QStandardPaths`는 Qt를 끌어와 CLI가 못 씀), 벤더 세그먼트는 `appauthor`가 Windows에서만 먹으므로 직접 join. 오버라이드를 걷어낸 실제 해석으로 두 경로가 겹치지 않음을 테스트. 매뉴얼의 "백업이 복사 하나" 근거 문단 재작성 및 한국어 카탈로그 재동기화(이번엔 msgid 기준 + fuzzy 0 단언)
- [115](20260728_115_one_user_data_directory.md) - **사용자 데이터를 한 디렉토리로**: 매뉴얼의 로그 경로 오류를 쫓아가 보니 레이아웃을 아는 곳이 네 군데였고 설정(`%APPDATA%`)과 로그(`~/PaleoBytes`)가 플랫폼마다 다른 곳에 있었음 — `utils/paths.py` 하나로 통합하고 설정을 Modan2와 같은 `preferences.json`으로. `CTHARVESTER_LOG_DIR`이 로그 파일만 옮기고 UI는 기본 위치를 보던 것도 이 통합으로 해소. 기본값 사본이던 `config/settings.yaml` 삭제 — 이미 6개 키가 어긋나 있었고 릴리스 빌드에는 번들된 적이 없어 출하 버전은 늘 파이썬 dict로 돌고 있었음. Modan2에서 물려받은 상수 때문에 DB 없는 프로그램이 매 실행마다 빈 `data/`·`backups/`를 만들던 것 제거(`--self-test`로 확인). 매뉴얼의 없는 키 이름 스키마, 세 가지로 틀린 로그 경로, 존재하지 않는 SQLite 캐시 복구 절, 죽은 픽스처, YAML을 쓰고 버튼 존재만 검사하던 import 테스트 정리 (+17 tests)
- [114](20260728_114_installer_aligned_with_paleobytes.md) - **인스톨러를 PaleoBytes 공통 규약에 맞춤**: `AppId`가 없어 신원이 `AppName`에서 파생되던 문제, Program Files(관리자 승격) 설치를 Modan2·PaperMeister와 같은 per-user `{localappdata}\PaleoBytes`로 이전. 버전을 하드코딩한 죽은 `CTHarvester.iss` 사본과 취소해도 디렉토리를 남기는 `InitializeSetup` 삭제, `SetupIconFile`이 존재하지 않는 경로였음을 치환 재연으로 발견. 규약의 근거였던 PaperMeister 커밋이 정작 CTHarvester가 예외임을 확인하지 않았다는 점이 관찰거리
- [113](20260727_113_one_changelog.md) - **변경 이력 단일화**: 릴리스 후 매뉴얼에 버전이 반영되는지 확인하다가, `docs/manual/changelog.rst`가 `CHANGELOG.md`와 별개로 손으로 관리되던 두 번째 이력이고 beta.3이 누락됐음을 발견 — myst include로 정본 하나만 남김. devlog 107의 "myst를 켜면 어떻게 되는가" 가정이 6시간 만에 실제로 시험되어 통과, 다만 디렉토리 경계가 닿지 않는 `docs/manual/` 내부는 `**.md` 배제로 방어(Modan2 선례)
