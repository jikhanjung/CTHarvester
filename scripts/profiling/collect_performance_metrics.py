...
def benchmark_thumbnail_generation(sample_dir: str) -> dict:
    ...
    metrics = {
        ...
        "images_per_second": (
            round(images_count / elapsed, 2) if elapsed > 0 and images_count > 0 else 0
        ),
...
def collect_metrics(sample_dir: str) -> dict:
    ...
    if Path(sample_dir).exists():
        ...
        metrics["benchmarks"]["thumbnail_generation"] = {
            "success": False,
...
def main():
    ...
    parser.add_argument(
        "--compare-baseline", action="store_true", help="Compare with baseline metrics"
    )
